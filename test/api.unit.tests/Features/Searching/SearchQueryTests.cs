...
public class SearchQueryTests {
    ...
    public async Task Should_return_results() {
        var query = new SearchQuery.Query("tableName", "return,values", new(new()), new());
        ...
        var result = await handler.Handle(query, CancellationToken.None) as JsonHttpResult<ApiResponseContract<IReadOnlyCollection<SearchResponseContract>>>;
    ...
    public async Task Should_handle_invalid_columns() {
        var query = new SearchQuery.Query("table_not_found", "not_found", new(new()), new());
