...
public class SearchQueryValidationTests {
    ...
    public async Task Should_fail_with_no_table_name() {
        var query = new SearchQuery.Query(string.Empty, "return,values", new(new()), new());

        var handler = new SearchQuery.ValidationBehavior<SearchQuery.Query, IResult>(mediator, _logger);
        var result = await handler.Handle(query, delegateMock.Object, CancellationToken.None) as JsonHttpResult<ApiResponseContract<IReadOnlyCollection<SearchResponseContract>>>;
    ...
    public async Task Should_fail_with_jerk_table_name() {
        var query = new SearchQuery.Query("jerk", "return,values", new(new()), new());
        ...
        var handler = new SearchQuery.ValidationBehavior<SearchQuery.Query, IResult>(_mediator.Object, _logger);
        var result = await handler.Handle(query, delegateMock.Object, CancellationToken.None) as JsonHttpResult<ApiResponseContract<IReadOnlyCollection<SearchResponseContract>>>;
    ...
    public async Task Should_fail_with_no_return_values() {
        var query = new SearchQuery.Query("table", string.Empty, new(new()), new());
    ...
    public async Task Should_fail_with_jerk_return_value() {
        var query = new SearchQuery.Query("table", "jerk", new(new()), new());
    ...
    public async Task Should_fail_with_empty_options() {
        var query = new SearchQuery.Query("table_not_found", "return,values", null, new());
    ...
    public async Task Should_call_next_with_no_errors() {
        var query = new SearchQuery.Query("table_not_found", "return,values", new(new()), new());
