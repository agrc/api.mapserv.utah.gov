...
namespace api.tests.Features.Geocoding {
    public class DeliveryPointTests {
        private readonly IDictionary<string, List<GridLinkable>> _deliveryPoints = new Dictionary<string, List<GridLinkable>>(1);
        private readonly UspsDeliveryPointLocation.Handler _handler;

        public DeliveryPointTests() {
            _deliveryPoints.Add("84114", new GridLinkable[] { new UspsDeliveryPointLink(84114, "grid", "place", 1, 1) }.ToList());

            var mockCache = new Mock<ILookupCache>();
            mockCache.Setup(x => x.UspsDeliveryPoints).Returns(_deliveryPoints);

            _handler = new UspsDeliveryPointLocation.Handler(mockCache.Object, new Mock<IMediator>().Object);
        }
