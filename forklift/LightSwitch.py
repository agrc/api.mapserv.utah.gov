...
class LightSwitch(object):
    def __init__(self, server):
        required_fields = (
            "Required information for connecting to ArcGIS Server do not exist. "
        )
        ...
        base_url = "{}://{}:{}/arcgis/admin".format(
            server["protocol"], server["machineName"], server["port"]
        )
    ...
    def ensure_services(self, what: str, affected_services: list = []):
        """ensures that affected_services are started or stopped with 5 attempts.
        ...
        while len(affected_services) > 0 and self.tries >= 0:
            ...
            affected_services = [
                act_on_service(service) for service in affected_services
            ]
            affected_services = [
                service for service in affected_services if service is not None
            ]
    ...
    def validate_service_state(self):
        ...
        for info in service_infos:
            if info["folderName"] == "/":
                ...
                service_path = "{}/{}.{}".format(
                    info["folderName"], info["serviceName"], info["type"]
                )

            service_status = self._fetch(
                "{}/{}/status".format(self.services_url, service_path)
            )
    ...
    def _return_false_for_status(self, json_response):
        ...
        """
        if (
            "status" in list(json_response.keys())
            and json_response["status"] == "error"
        ):
