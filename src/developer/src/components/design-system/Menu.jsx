...
const MenuLink = ({ to, children }) => {
  ...
  return (
    <NavigationMenu.Link
      asChild
      className={clsx(
        'group relative flex select-none items-center justify-between gap-1 px-3 py-2 font-bold leading-none text-wavy-900 outline-none',
        {
  ...
  return (
    <NavigationMenu.Root className="relative z-10 flex w-screen justify-start border-b border-dashed border-b-wavy-300 bg-wavy-200/50">
      ...
      <div className="absolute left-2 top-full flex w-full justify-start">
        <NavigationMenu.Viewport className="relative mt-2 h-[var(--radix-navigation-menu-viewport-height)] w-full overflow-hidden rounded border bg-white shadow data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:zoom-out data-[state=open]:zoom-in sm:w-[var(--radix-navigation-menu-viewport-width)]" />
      </div>
    </NavigationMenu.Root>
  );
};
  ...
  ({ className, children, title, ...props }, forwardedRef) => (
    <li className="leading-[.5em]">
      <NavigationMenu.Link asChild>
        <a
          className={clsx(
            'block select-none rounded p-3 text-base no-underline outline-none transition-colors hover:bg-slate-100 focus:shadow-[0_0_0_2px] focus:shadow-mustard-400',
            className
          )}
          {...props}
          ref={forwardedRef}
        >
          <div className="font-medium text-wavy-700">{title}</div>
          <p className="text-sm text-wavy-500">{children}</p>
        </a>
      </NavigationMenu.Link>
    </li>
  )
