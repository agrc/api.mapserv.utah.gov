  ...
  return (
    <section>
      ...
      <div className="mx-auto max-w-prose">
        <p>
          This page is not accessible to you for your UIC inventory submission
          process. If normal usage has brought you here, please click the{' '}
          <Link data-style="link" to="contact">
            Contact us
          </Link>{' '}
          link and let us know what happened. Otherwise, please go back to the
          main page and navigate to your item of interest.
        </p>
        <div className="my-4 rounded border border-red-300 bg-red-100/30 px-4">
          <h3 className="my-1 text-lg font-medium text-gray-700">
            Violated rules
          </h3>
          <ul className="mb-3">
            {rules.map((rule) => (
...
const UnhandledException = ({ error, routeError }) => {
  ...
  return (
    <section>
      ...
      <p>
        We are really sorry. There was an error in the application that caused
        it to crash. You may now{' '}
        <button data-style="link" onClick={() => navigate(-1)}>
          go back
        </button>{' '}
        to the previous page and try again or contact us to share the technical
        details with us from below.
      </p>
      <details className="mt-6">
        ...
        {routeError && (
          <>
            ...
            <pre
              id="route"
              className="overflow-auto text-base text-gray-400"
            >{`${routeError}`}</pre>
          </>
        )}
        {error && error.message ? (
        ...
        {error && !error.message && !error.stack ? (
          <pre className="overflow-auto text-base text-gray-400">
            {error.toString()}
          </pre>
        ) : null}
      </details>
