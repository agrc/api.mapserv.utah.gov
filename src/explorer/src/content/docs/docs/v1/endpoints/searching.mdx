...
- `shape@` provides a geometry simplified with a tolerance of 10 meters. The search endpoint is intended for map extents and general data extraction, so this geometry may not preserve every detail of the source feature. For more precise geometry or access to additional geometry features, use the feature services available in [SGID on ArcGIS](https://opendata.gis.utah.gov/).
