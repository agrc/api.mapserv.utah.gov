  ...
  useEffect(() => {
    setUtahHeaderSettings({
      domLocationTarget: {
        ...
        cssSelector: '#utah-header-placeholder',
      },
      title: 'UGRC API',
      // size: 'SMALL',
      titleURL: '/',
  ...
  return (
    <main className="w-full h-full flex flex-col">
      <header
        ref={headerRef}
        className="w-full"
        id="utah-header-placeholder"
      ></header>
      <div className="flex flex-1 mx-auto w-full">
        <section className="max-w-prose lg:max-w-none grid mx-auto flex-1 gap-6 sm:px-20 px-10 w-1/2 my-auto">
          <h2 className="text-wavy-800 pt-4">Sign in to your account</h2>
          ...
          <div className="flex justify-center">
            <button
              type="button"
              className="text-xl px-10 py-3 h-12 min-h-[3rem] active:scale-95 active:shadow-inner flex w-fit cursor-pointer select-none items-center justify-center rounded-full border-wavy-600 bg-wavy-500 text-mustard-100 focus:border-wavy-700 focus:ring-wavy-700 hover:border-wavy-600 hover:bg-wavy-700 transition-all duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-opacity-50 hover:text-white"
            >
              <svg
        ...
        <section className="relative hidden lg:flex flex-col flex-1 bg-gradient-to-b to-mustard-400 via-wavy-400 from-wavy-700 w-1/2">
          <img
            className="absolute left-0 top-0 h-full w-full"
            src="/sparkles.png"
            alt=""
          ></img>
          <h2 className="text-white mx-auto mt-12">Location Matters</h2>
          <h3 className="text-white mx-auto">Let data take you places</h3>
        </section>
      </div>
      <footer
        ref={footerRef}
        className="w-full h-32 block"
        id="utah-footer-placeholder"
      ></footer>
    </main>
  );
}
