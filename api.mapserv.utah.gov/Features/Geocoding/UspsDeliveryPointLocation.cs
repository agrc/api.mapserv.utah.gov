    ...
    public class UspsDeliveryPointLocation
    {
        ...
        public class Handler : IRequestHandler<Command, Candidate>
        {
            ...
            public async Task<Candidate> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                _driveCache.UspsDeliveryPoints.TryGetValue(request.Address.Zip5.Value.ToString(), out List<GridLinkable> items);
                ...
                var _reproject = new Reproject.Command(new PointReprojectOptions(26912, request.Options.SpatialReference,
                                                                                       new[]
                                                                                       {
                    deliveryPoint.X,
                    deliveryPoint.Y
                ...
                var pointReprojectResponse = await _mediator.Send(_reproject);
