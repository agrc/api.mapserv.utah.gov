...
namespace WebAPI.API.Commands.ArcGIS
{
    ...
    {
        private IAoInitialize m_AoInit = new AoInitialize();
        ...
        public string LicenseMessage()
        {
            ...
            {
                prodStatus = ReportInformation(AoInit as ILicenseInformation,
                                 AoInit.InitializedProduct(),
            ...
            {
                string info = ReportInformation(AoInit as ILicenseInformation, item.Key, item.Value);
        ...
        public void ShutdownApplication()
        {
            ...
            foreach (KeyValuePair<esriLicenseExtensionCode, esriLicenseStatus> item in m_extensionStatus)
            {
                if (item.Value == esriLicenseStatus.esriLicenseCheckedOut)
                    AoInit.CheckInExtension(item.Key);
            }
        ...
        {
            return AoInit.IsExtensionCheckedOut(code);
        }
        ...
        public void RemoveExtensions(params esriLicenseExtensionCode[] requestCodes)
        {
            ...
            foreach (esriLicenseExtensionCode code in requestCodes)
            {
                ...
                {
                    if (AoInit.CheckInExtension(code) == esriLicenseStatus.esriLicenseCheckedIn)
                    {
        ...
        public esriLicenseProductCode InitializedProduct
        {
            get
            {
                try
                {
                    return AoInit.InitializedProduct();
                }
                catch
                {
                    return 0;
        ...
        private bool Initialize()
        {
            ...
            ILicenseInformation licInfo = (ILicenseInformation)AoInit;
            ...
            foreach (int prodNumber in m_requestedProducts)
            {
                ...
                esriLicenseStatus status = AoInit.IsProductCodeAvailable(prod);
                if (status == esriLicenseStatus.esriLicenseAvailable)
                {
                    status = AoInit.Initialize(prod);
                        ...
                        status == esriLicenseStatus.esriLicenseCheckedOut)
                    {
                        ...
                        currentProduct = AoInit.InitializedProduct();
                    }
                }
        ...
        private bool CheckOutLicenses(esriLicenseProductCode currentProduct)
        {
            ...
            if (m_requestedExtensions != null && currentProduct != 0)
            {
                ...
                {
                    esriLicenseStatus licenseStatus = AoInit.IsExtensionCodeAvailable(currentProduct, ext);
                    if (licenseStatus == esriLicenseStatus.esriLicenseAvailable)//skip unavailable extensions
                    {
                        licenseStatus = AoInit.CheckOutExtension(ext);
                    }
